...
const api: EditorApi = {
  ...
  sttTranscribe: (opts: SttTranscribeOptions) => ipcRenderer.invoke('stt:transcribe', opts),
  sttCancel: (jobId) => ipcRenderer.invoke('stt:cancel', jobId),
  onSttProgress: (cb) => {
    const listener = (_e: Electron.IpcRendererEvent, p: SttProgressEvent) => cb(p)
