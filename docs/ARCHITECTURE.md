...
### 6.1 렌더 실행 위치 — 확정 (1.5 스파이크, 2026-07-02)

**채택: 보이는 렌더러 프로세스의 OffscreenCanvas.** 프리뷰와 완전히 동일한 GL 환경에서 렌더 → WYSIWYG 보장이 최우선. 내보내기 중 편집 UI는 오버레이로 잠근다.

검토했던 후보:

| 후보 | 장점 | 단점 |
|------|------|------|
| **보이는 렌더러 (채택)** | 프리뷰와 동일 환경(WYSIWYG), 구현 단순 | 내보내기 중 편집 불가(오버레이로 잠금) |
| 숨김 BrowserWindow | 편집 UI와 격리 | 창 관리 복잡도, 메모리 추가 — 처리량 문제 발생 시 이전 |
| UtilityProcess + 소프트웨어 렌더 | 인코더(메인)와 가까움 | GPU 접근 제약, 프리뷰와 환경 상이 → WYSIWYG 리스크 |

### 6.2 프레임 전송 대역폭 — 실측 결과

1080p30 RGBA 원시 프레임 ≈ **초당 250MB** 필요. 스파이크 실측(Apple Silicon 개발 머신):
- `renderer → main → FFmpeg stdin` (IPC 구조 복제 + stdin drain 백프레셔): **파이프 처리량 497MB/s**
- 1080p30 타임라인 내보내기 **2.0× 실시간** — 목표(≤2×) 충족, 병목은 파이프가 아니라 렌더+인코딩
- **재검토 트리거**: 장편/4K 에서 속도 미달 시 YUV 변환 전송(대역폭 ~50%↓) 또는 숨김 창 + 직접 파이프로 이전

### 6.3 색공간 규칙 — 실측 결정 (WYSIWYG 핵심)

- **실측**: Chromium(WebCodecs→WebGL 텍스처 경로)은 BT.601 계열(bt470bg/smpte170m) 태그를 무시하고 **BT.709 로 변환**한다 (SMPTE 바 녹색 189→159 로 확인).
- **규칙**: ① BT.709 가 아닌(또는 SD 미태깅) 소스는 0.4 호환 프록시에서 BT.709 로 정규화, ② 내보내기 RGB→YUV 변환은 `scale=out_color_matrix=bt709` 로 고정하고 스트림에 bt709 태깅.
- 결과: 원본 대비 출력 SSIM 0.983 (e2e 자동 검증).
